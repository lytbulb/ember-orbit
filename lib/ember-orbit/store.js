...
var Store = Source.extend({
  ...
  findLinked: function(type, id, field) {
    console.log("findLinked", [type, id, field]);
    var _this = this;
    this._verifyType(type);
    id = this._normalizeId(id);
    ...
    return this._request(promise.then(function(linkedRecords){
      console.log("findLinked(" + [type, id, field].join("/") + ") => ", linkedRecords);
  ...
  _didTransform: function(operation, inverse) {
   ...
   if(isArray(operation.path[1])) debugger;
