...
import HasOneManager from './has-one-manager';
...
var Store = Source.extend({
  ...
  init: function() {
    ...
    this._hasOneManager = HasOneManager.create({
      store: this
    });
  },
  ...
  _didTransform: function(operation, inverse) {
   ...
   console.log('_didTransform', operation);
    ...
    if (path.length === 3) {
      ...
      var typeMap = this.typeMapFor(path[0]);
      var record = typeMap.records[path[1]];
      if(record){
    ...
    this._hasOneManager.recordDidChange(record, operation);
