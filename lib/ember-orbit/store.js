...
import { copyInto } from './lib/operations';
...
var Store = Source.extend({
  ...
  findLink: function(type, id, field) {
    ...
    return this.orbitSource.findLink(type, id, field);
  },
  ...
  retrieveOrStub: function(type, id){
    var records = this.retrieve(type, id);

    if(Ember.isArray(id)){
      return records;
    }
    else {
      return records || this._lookupRecord(type, id);
    }
  },
  ...
  retrieveLink: function(type, id, field) {
    ...
    if (linkType && relatedId) {
      return this.retrieveOrStub(linkType, relatedId);
    }
  },

  isLinkLoaded: function(type, id, field){
    this._verifyType(type);
    id = this._normalizeId(id);
    ...
    var relatedId = this.orbitSource.retrieve([type, id, '__rel', field]);
    if (linkType && relatedId){
      if(isArray(relatedId)){
        return relatedId.any(function(relId){
          return !this.orbitSource.retrieve([linkType, relId]);
        });
      }
      else {
        return this.orbitSource.retrieve([linkType, relatedId]);
      }
    }
  },
  ...
  retrieveLinks: function(type, id, field) {
    ...
    if (linkType && Ember.isArray(relatedIds) && relatedIds.length > 0) {
      return this.retrieveOrStub(linkType, relatedIds);
    }
  },
  ...
  _didTransform: function(operation, inverse) {
    var _this = this;
   console.log('_didTransform', operation);
    ...
    else if (path.length === 2 && operation.value) {
      copyInto(operation.value, record, get(this, 'schema'), path[0]);
    }
