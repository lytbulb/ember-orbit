...
var RecordArrayManager = Ember.Object.extend({
  ...
  _linkWasAdded: function(record, key, value) {
    console.log("_linkWasAdded", arguments);
    var type = record.constructor.typeKey;
    var store = get(this, 'store');
    var linkType = get(store, 'schema').linkProperties(type, key).model;

    if (linkType) {
      var relatedRecord = store.retrieveOrStub(linkType, value);
      var links = get(record, key);

      store.then(function(){
        var loaded = store.retrieve(linkType, value);
        if(!loaded) {
          // todo - find a more elegant way to do this... it works because it forces firebase-orbit
          // to add a listener for the missing record, when it's added to firebase the listener fires
          // and the store is updated
          store.find(linkType, value).then(null, function(){});
        }
      });

      if (links && relatedRecord) {
        console.log("_linkWasAdded.added link");
  ...
  _linkWasRemoved: function(record, key, value) {
    console.log("linkWasRemoved", arguments);
    debugger
    var type = record.constructor.typeKey;
    var store = get(this, 'store');
    var linkType = get(store, 'schema').linkProperties(type, key).model;
  ...
  registerHasManyArray: function(hasManyArray, type){
    var recordArrays = this.filteredRecordArrays.get(type);
    recordArrays.push(hasManyArray);
  },
