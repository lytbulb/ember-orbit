...
var Model = Ember.Object.extend(Ember.Evented, {
  ...
  getLink: function(field) {
    ...
    // todo - where should this live? store.linkExists?
    var relatedId = store.orbitSource.retrieve([type, id, '__rel', field]);
    var linkType = get(store, 'schema').linkProperties(type, field).model;
    var relatedRecordExists = store.orbitSource.retrieve([linkType, relatedId]);

    console.log('what is relatedRecord: ', relatedRecord);

    if(!relatedRecordExists){
      store.findLink(type, id, field).then(function(linkId){
        if(linkId) hasOneObject.reload();
      });
    }
  ...
  getLinks: function(field) {
    ...
    var linkType = get(store, 'schema').linkProperties(type, field).model;
    var requiresLoad = relatedRecords.any(function(relatedRecord) {
      return !store.orbitSource.retrieve([linkType, relatedRecord.primaryId]);
    });
