...
var Model = Ember.Object.extend(Ember.Evented, {
  ...
  getLink: function(field) {
    ...
    store._hasOneManager.add(hasOneObject);
  ...
  getLinks: function(field) {
    ...
    store._recordArrayManager.registerHasManyArray(hasManyArray);

    var linkType = get(store, 'schema').linkProperties(type, field).model;
    var requiresLoad = relatedRecords.any(function(relatedRecord) {
      return !store.retrieve(linkType, relatedRecord.primaryId);
    });

    if (requiresLoad) {
      console.log("lazy loading " + [type, field].join("/"));
      hasManyArray.reload();
    }
